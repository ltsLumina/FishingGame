namespace FishingRod
{
	...
	UFishingRod GenerateRod(UObject Outer, URodItem RodItem)
	{
		UFishingRod NewRod = NewObject(Outer, UFishingRod);
		NewRod.RodItem = RodItem;
		NewRod.Traits = RodItem.RodData.Traits.IsCurated ? RodItem.RodData.Traits.CuratedTraits : RollTraits(NewRod);
		return NewRod;
	}

	TArray<TSubclassOf<UTrait>> RollTraits(UFishingRod Rod)
	{
		auto RodItem = Rod.RodItem;
		auto RodData = RodItem.RodData;
		auto Traits = RodData.Traits;
	...
	FString GetStatsString(URodItem RodItem)
	{
		auto RodData = RodItem.RodData;
...
class URodItem : UItem
{
	UPROPERTY(Category = "Rod | Info", ExposeOnSpawn, SaveGame)
	FRodItemData RodData;

	default RodData.BaseStats.Add(GameplayTags::Stat_Fishing_ReelSpeed, 0.0f);
	default RodData.BaseStats.Add(GameplayTags::Stat_Fishing_BiteRate, 0.0f);
	default RodData.BaseStats.Add(GameplayTags::Stat_Fishing_CatchChance, 0.0f);
	default RodData.BaseStats.Add(GameplayTags::Stat_Fishing_Luck, 0.0f);

	default RodData.Traits.TraitCountChances.Add(20); // 0 traits
	default RodData.Traits.TraitCountChances.Add(25); // 1 trait
	default RodData.Traits.TraitCountChances.Add(30); // 2 traits
	default RodData.Traits.TraitCountChances.Add(20); // 3 traits
	default RodData.Traits.TraitCountChances.Add(5);  // 4 traits
